...
class FileController extends Controller
{
    ...
    /**
     * @Route("/intranet/hrm/export/leave-requests", name = "premier_test")
     */
    public function xlsExportAxtion()
    {
        ...
        $i=2;
        foreach ($listLeave as $leave)
        {
            $newSpreadsheet->getActiveSheet()->setCellValue('A'.strval($i),strval($leave->getType()));
            $newSpreadsheet->getActiveSheet()->setCellValue('B'.strval($i),$leave->getEmployee()->getUserName());
            $newSpreadsheet->getActiveSheet()->setCellValue('C'.strval($i),$leave->getStartDate()->format('d-m-Y'));
            $newSpreadsheet->getActiveSheet()->setCellValue('D'.strval($i),$leave->getEndDate()->format('d-m-Y'));
            $newSpreadsheet->getActiveSheet()->setCellValue('E'.strval($i),$leave->getReason());        
            $i++;
        }
        ...
        $dispositionHeader = $response->headers->makeDisposition(
            ...
            'leave_requests_'.strval(rand()).'.xlsx'
        );
